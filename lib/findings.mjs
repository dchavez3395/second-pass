...
export const CHECKS = path.join(ROOT, 'checks.jsonl');
...
export function loadFindings() {
  ...
  for (const f of readdirSync(DIAG).filter((n) => n.endsWith('.json')).sort()) {
    ...
    siteMeta[site] = { url: rec.url, title: rec.title || '', shot, viewport: rec.viewport || null };
  }
  // Findings the model raised where axe has no rule (checks.mjs), tagged kind 'model'.
  if (existsSync(CHECKS)) {
    const latest = {};
    for (const line of readFileSync(CHECKS, 'utf8').split('\n')) {
      if (!line.trim()) continue;
      try { const c = JSON.parse(line); latest[c.id] = c; } catch {}
    }
    const perCheck = {};
    for (const c of Object.values(latest)) (perCheck[`${c.site}|${c.check}`] ||= []).push(c);
    for (const arr of Object.values(perCheck)) {
      arr.forEach((c, i) => {
        const meta = siteMeta[c.site] || {};
        const messages = [c.reason];
        if (c.suggestion) messages.push(`Suggested: ${c.suggestion}`);
        out.push({
          id: c.id,
          site: c.site,
          url: meta.url || '',
          ...
          wcag: [c.criterion],
          tags: [],
          impact: c.check === 'alt-text' ? 'serious' : 'moderate',
          target: c.target,
          html: c.html || '',
          messages,
          box: c.box ? { ...c.box, text: c.alt !== undefined ? (c.alt === '' ? '(empty alt)' : c.alt) : c.text || '' } : null,
          shot: meta.shot || null,
          crop: c.crop && existsSync(path.join(CROPS, c.crop)) ? c.crop : null,
          viewport: meta.viewport || null,
          nodeIndex: i,
          nodeCount: arr.length,
          model: { problem: c.problem, suggestion: c.suggestion || '', confidence: c.confidence, model: c.model, alt: c.alt, text: c.text, href: c.href, src: c.src },
        });
      });
    }
  }
...
export function groupKey(f) {
  if (f.kind === 'model') return [f.site, 'model', f.rule, f.model.problem].join('§');
...
export function groupFindings(findings, proposals = {}) {
  ...
  for (const f of findings) {
    ...
    if (!groups.has(key)) {
      ...
      const parts = [];
      if (f.kind === 'model') {
        parts.push(String(f.model.problem || '').replace(/-/g, ' '));
      } else if (f.rule === 'color-contrast' || f.rule === 'link-in-text-block') {
      ...
      groups.set(key, {
        ...
        impact: f.impact,
        label: parts.join(' · '),
        why: f.kind === 'model' ? (f.model.model ? `Raised by ${f.model.model} — no axe rule covers this. Confirm or ignore each one.` : 'Raised by a deterministic check — no axe rule covers this.') : f.messages[0] || '',
        ids: [],
        measured: null,
      });
    }
...
/** The scan record per site: axe totals, page shape, verdict. */
export function loadSiteRecords() {
  const out = {};
  if (!existsSync(DIAG)) return out;
  for (const f of readdirSync(DIAG).filter((n) => n.endsWith('.json'))) {
    try {
      const rec = JSON.parse(readFileSync(path.join(DIAG, f), 'utf8'));
      if (!rec.ok) continue;
      const site = new URL(rec.url).hostname.replace(/^www\./, '');
      out[site] = { site, url: rec.url, title: rec.title, axe: rec.axe, shape: rec.shape, verdict: rec.verdict, scannedAt: rec.scannedAt };
    } catch {}
  }
  return out;
}
